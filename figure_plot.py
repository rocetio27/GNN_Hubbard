...
for iu, U in enumerate(us):
    for it, t in enumerate(ts):
        # 5.1) 샘플 데이터 생성
        create_and_save_data(4, U.item(), t.item(), "tmp")

        # 5.2) 노드/엣지 피처 로드
        node_features_all = torch.load(os.path.join(module.input_dir, "node_features_all_tmp.pt"), map_location=device)
        edge_attr_all     = torch.load(os.path.join(module.input_dir,     "edge_attr_all_tmp.pt"), map_location=device)
        edge_index_all    = torch.load(os.path.join(module.input_dir,    "edge_index_all_tmp.pt"), map_location=device)
...
plt.tight_layout()
plt.savefig('error_map_with_samples_positive.png', dpi=900)
plt.show()